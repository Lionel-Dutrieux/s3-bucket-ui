 ...
 */
export function createSelectColumn(t: ColumnsT): ColumnDef<BrowserEntry> {
  return {
    id: "select",
    enableSorting: false,
    ...
    cell: ({ row, table }) => {
      ...
      return (
        <Checkbox
          ...
          aria-label={t("selectEntry", { name: row.original.name })}
          className={cn(
            "transition-opacity",
            !row.getIsSelected() &&
          ...
          <>
            {entry.kind === "folder" ? (
              <a
                ...
                aria-label={t("downloadZipAria", { name: entry.name })}
                title={t("downloadZipTitle")}
              >
              ...
              <a
                ...
                className={ROW_ACTION_CLASS}
                aria-label={t("downloadAria", { name: entry.name })}
